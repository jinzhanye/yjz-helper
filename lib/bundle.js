...
function removeHash(input) {
  var hashStart = indexOf$3(input).call(input, '#');

  if (hashStart !== -1) {
    input = slice$2(input).call(input, 0, hashStart);
  }
...
function getHash(url) {
  ...
  var hashStart = indexOf$3(url).call(url, '#');

  if (hashStart !== -1) {
    hash = slice$2(url).call(url, hashStart);
  }
...
function parse(str, options) {
  ...
  str = trim$2(str).call(str).replace(/^[?#&]/, '');
  ...
  forEach$2(_context = str.split('&')).call(_context, function (pair) {
...
function stringify(query, options) {
  ...
  return filter$2(_context2 = map$2(_context3 = keys$2(query || {})).call(_context3, function (key) {
    ...
    return null;
  })).call(_context2, function (params) {
    return params;
  }).join('&');
}
function parseUrl(input, options) {
  ...
  var url = removeHash(input).split('?')[0] || '';
  var urlObject = parse$1(input);
  var query = parse(urlObject.query, options);
  return {
    url: url,
    query: query
  };
...
function stringifyUrl(input, options) {
  ...
  var _parseUrl = parseUrl(input.url),
      url = _parseUrl.url,
      query = _parseUrl.query;

  var hash = getHash(input.url);
  var queryString = stringify(assign$2({}, query, input.query), options);
...
export { parse, parseUrl, stringify, stringifyUrl };
